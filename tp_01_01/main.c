...
pid_t crear_hijo(int tiempo_vida){
	...
	else  			  /*codigo del hijo */
	{
		...
		while(1)
		{
			...
			sprintf(mi_string,"Hijo <%d> de <%d>, Vivo hace <%d> mseg\n" ,mi_pid_hijo,padre,tdiff/1000);
...
void time_stamp(struct timeval tiempo, int hora,int minutos,int segundos){

	hora= (int)(tiempo.tv_usec/3600000);
	minutos=(int)((tiempo.tv_usec-hora*3600000)/60);
	segundos=tiempo.tv_usec-(hora*3600000+minutos*60);
...
int main(int argc, char const *argv[]) {
	...
	if((fd= open (NOM_ARCHIVO,O_CREAT | O_APPEND |  O_RDWR,S_IRWXG ))==-1)

	{
		perror("open: "); //escibe el tipo de error por el que se proboco la falla
		exit(2);
	}
	key = ftok("./registro.log", 'E');

	sem_archivo = crea_sem(key,1); //key y valor inical del semaforo, lo creo liberado para que el primer hijo a escribir lo pueda tomar
